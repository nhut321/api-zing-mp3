...
const { CLOUD_NAME, CLOUDINARY_API_KEY, CLOUDINARY_API_SECRET, API_KEY } =
  process.env;
...
module.exports = {
  ...
  GET_NAME_SINGER: async (req, res) => {
    try {
      ...
      const features = new ApiFeatures(
        ...
        mongooseMusic.find({
          slug_subscribe: { $regex: handleSlug(_singer), $options: "i" },
        }),
        req.query
      ).sorting();
      const result = await features.query;
  ...
  GET_NAME_MUSIC: async (req, res) => {
    try {
      ...
      const result = await mongooseMusic.findOne({
        slug_name_music: handleSlug(_name),
      });
    ...
    try {
      ...
      if (_type === "million") {
        features = new ApiFeatures(
          mongooseMusic
            .find({ view: { $gte: 1000000, $lte: 999999999 } })
            .sort({ view: -1 }),
          req.query
        );
      }
  ...
  TOP_FAVORITE: async (req, res) => {
    try {
      ...
      if (_type === "million") {
        features = new ApiFeatures(
          mongooseMusic
            .find({ favorite: { $gte: 1000000, $lte: 999999999 } })
            .sort({ favorite: -1 }),
          req.query
        );
      }
  ...
  SEARCH: async (req, res) => {
    try {
      ...
      const queryString = {
        id_account: id,
        $or: [{ slug_name_music: { $regex: query, $options: "$i" } }],
      };
      const features = new ApiFeatures(
